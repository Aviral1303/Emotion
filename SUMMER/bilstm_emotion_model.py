#!/usr/bin/env python3
# bilstm_emotion_model.py - Emotion classification from motion capture data using BiLSTM with PyTorch
...
class BiLSTMEmotionClassifier(nn.Module):
    """
    BiLSTM model for emotion classification using PyTorch.
    """
    def __init__(self, input_dim, hidden_dim=128, num_classes=3, dropout=0.3):
        ...
        self.lstm2 = nn.LSTM(
            input_size=hidden_dim * 2,  # Bidirectional so *2
            hidden_size=hidden_dim // 2,  # Smaller for second layer
            bidirectional=True,
            batch_first=True
        )
        ...
        # Dense layers
        self.dense1 = nn.Linear(hidden_dim, 64)  # hidden_dim because bidirectional * 2 / 2
        self.relu = nn.ReLU()
    ...
    def forward(self, x):
        ...
        dense1_out = self.dropout3(dense1_out)
...
def plot_training_history(history, save_path='bilstm_training_plot.png'):
    """
...
def main():
    ...
    try:
        ...
        bvh_files = load_bvh_files(data_dir, limit=150)  # Increased from 100 to 150 files
        ...
        # Create model
        model = BiLSTMEmotionClassifier(input_dim=input_dim, num_classes=num_classes)
        model.to(device)
